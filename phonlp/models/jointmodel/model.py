...
class JointModel(BertPreTrainedModel):
    ...
    def pos_forward(self, tokens_phobert, first_subword, sentlens, use_soft_pos=False, upos=None):
        ...
        # packed dim:4      0:450*768      1:75
        phobert_emb = pack(phobert_emb)
        # dim:4 0: 450*768   1: 75
        inputs += [phobert_emb]
    ...
    def ner_forward(self, tokens_phobert, first_subword, word_mask, sentlens, tags):
        def pack(x):
            return pack_padded_sequence(x, sentlens, batch_first=True)
        # inputs ,softmax
        # token_phobert: 8*256,B*H , first_subword: 8*31,B*T , sentlens: list,8, use_soft_pos unknown   
        # inputs:PackedSentence  4 dim , 0: 154*768  ,1 : 31
        # pos_dis: 8*31*24
        ...
        def pad(x):
            return pad_packed_sequence(PackedSequence(x, phobert_emb.batch_sizes), batch_first=True)[0]
        # upos_embed_matrix_dup 8*24*100 ,upos_emb_matrix_ner: 24*100 , pos_dis.size(0):8 
        upos_embed_matrix_dup = self.upos_emb_matrix_ner.repeat(pos_dis.size(0), 1, 1)
        # 8*31*100                     8*31*24      8*24*100 
        pos_emb = torch.matmul(pos_dis, upos_embed_matrix_dup)
        # PackedSequence 0: 154*100 1: 31(sent_len)
        pos_emb = pack(pos_emb)
        # inputs:PackedSentence  4 dim , 0: 154*768  ,1 : 31
        # pos_emb:PackedSequence         0: 154*100  ,1 : 31(sent_len)
        inputs += [pos_emb]
        # 154,868
        inputs = torch.cat([x.data for x in inputs], 1)
        # 154,868                    154,868               size:[858] 
        ...
        # 12,12                     8,31,12  8,31,tpye:ture  8,31          
